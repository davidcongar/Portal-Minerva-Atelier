...
def forgot_password_email(recipient_email,unique_code):
    try:
        ...
        send_html_email(
            subject="Cambio de contraseña - Portal Minerva Atelier",
...
def new_user_email(recipient_email,contrasena):
    try:
        # Enviar correo al cliente
        send_html_email(
            subject="Portal Minerva Atelier",
            recipient_email=recipient_email,
            template="partials/system/email_template.html",
            body_content="Se acaba de crear tu usuario para la aplicación Portal Minerva Atelier.",
            details_list=[
